...
const t = {
  de: {
    ...
    fastReplyText: <>Wir antworten auf alle Anfragen innerhalb von <strong style={{ color: "#4ADE80" }}>24 Stunden</strong> – auch am Wochenende.</>,
  ...
  en: {
    ...
    fastReplyText: <>We respond to all enquiries within <strong style={{ color: "#4ADE80" }}>24 hours</strong> – including weekends.</>,
...
export default function Kontakt() {
  ...
  const handleSubmit = async () => {
    ...
    await fetch("/api/send-email", {
      ...
      body: JSON.stringify({
        type: "kontakt",
        data: { name: form.name, email: form.email, betreff: form.betreff, nachricht: form.nachricht }
      })
    });
    setLoading(false);
    setSent(true);
        ...
        <div className="kontakt-grid" style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 40, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ background: `linear-gradient(135deg, ${NAVY}, ${BLUE})`, borderRadius: 20, padding: 28, color: "white" }}>
              ...
              <p style={{ fontSize: "0.88rem", opacity: 0.8, lineHeight: 1.7, margin: 0 }}>{tx.fastReplyText}</p>
            </div>
