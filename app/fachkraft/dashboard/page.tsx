...
export default function FachkraftDashboard() {
  ...
  const loadNachrichten = async (fachkraftId: string) => {
    ...
    setNachrichten(msgs || []);
    const arbeitgeberIds = [...new Set((msgs || []).map((m: any) => m.von_id === fachkraftId ? m.an_id : m.von_id))].filter(id => id !== fachkraftId);
    if (arbeitgeberIds.length > 0) {
      const { data: arbeitgeber } = await supabase.from("arbeitgeber").select("id, einrichtung_name, email").in("id", arbeitgeberIds as string[]);
      const map: any = {};
  ...
  const handleDeleteAccount = async () => {
    if (!window.confirm("Bist du sicher? Dein Account wird unwiderruflich geloescht.")) return;
    ...
    else alert("Fehler beim Loeschen. Bitte kontaktiere uns unter hallo@kitabridge.de");
  };
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#F0F4F9", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ color: NAVY, fontWeight: 600 }}>Laedt...</div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#F0F4F9", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .tab-btn.active { color: #1A3F6F; border-bottom-color: #1A3F6F; }
        .tab-btn:hover { color: #1A3F6F; }
        ...
        <div className="header-inner" style={{ maxWidth: 760, margin: "0 auto", padding: "0 24px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {offeneAnfragen.length > 0 && (
              <div style={{ background: RED, color: "white", borderRadius: 50, padding: "2px 9px", fontSize: "0.75rem", fontWeight: 800 }}>{offeneAnfragen.length} neu</div>
            )}
            <button onClick={handleLogout} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", color: "white", padding: "6px 14px", borderRadius: 8, cursor: "pointer", fontSize: "0.82rem", fontFamily: "'DM Sans', sans-serif" }}>Ausloggen</button>
          </div>
        </div>
      </div>
      ...
      <div className="main-pad" style={{ maxWidth: 760, margin: "0 auto", padding: "24px 24px" }}>
        ...
        <div style={{ background: `linear-gradient(135deg, #0D1B2A 0%, ${NAVY} 60%, #1a5276 100%)`, borderRadius: 22, padding: "24px 28px", marginBottom: 20, position: "relative", overflow: "hidden", boxShadow: "0 16px 48px rgba(26,63,111,0.3)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16, position: "relative" }}>
            <div style={{ width: 58, height: 58, borderRadius: 16, background: "rgba(255,255,255,0.12)", border: "2px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", fontWeight: 800, color: "white", flexShrink: 0 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "0.82rem", color: "rgba(255,255,255,0.55)", marginTop: 2 }}>{fachkraft?.qualifikation || "Paedagogische Fachkraft"}</div>
            </div>
            <div style={{ background: fachkraft?.status === "bestaetigt" ? "linear-gradient(135deg,#1E8449,#27AE60)" : "rgba(255,255,255,0.15)", borderRadius: 50, padding: "5px 14px", fontSize: "0.72rem", fontWeight: 800, color: "white", textTransform: "uppercase", letterSpacing: 1, whiteSpace: "nowrap", border: fachkraft?.status !== "bestaetigt" ? "1px solid rgba(255,255,255,0.2)" : "none" }}>
              {fachkraft?.status === "bestaetigt" ? "Verifiziert" : "In Pruefung"}
            </div>
          </div>
        ...
        <div style={{ background: "white", borderRadius: 14, padding: "14px 18px", marginBottom: 20, boxShadow: "0 2px 8px rgba(26,63,111,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: "0.78rem", color: "#9BA8C0", marginTop: 2 }}>
              {fachkraft?.aktiv_suchend ? "Du bist sichtbar fuer Kitas" : "Du bist gerade nicht sichtbar fuer Kitas"}
            </div>
          </div>
          <div onClick={async () => { const neu = !fachkraft.aktiv_suchend; await supabase.from("fachkraefte").update({ aktiv_suchend: neu }).eq("email", fachkraft.email); setFachkraft({ ...fachkraft, aktiv_suchend: neu }); }} style={{ width: 50, height: 26, borderRadius: 50, background: fachkraft?.aktiv_suchend ? GREEN : "#D1DAE8", cursor: "pointer", position: "relative", transition: "background 0.3s", flexShrink: 0 }}>
          ...
          <div style={{ padding: "20px 20px" }}>
            ...
            {activeTab === "uebersicht" && (
              <div>
                <div className="stats-row" style={{ display: "flex", gap: 10, marginBottom: 20 }}>
                  {[
                    { label: "Offene Anfragen", value: offeneAnfragen.length, color: offeneAnfragen.length > 0 ? RED : NAVY },
                    { label: "Angenommen", value: anfragen.filter(a => a.status === "akzeptiert").length, color: GREEN },
                    { label: "Nachrichten", value: nachrichten.length, color: BLUE },
                  ].map(stat => (
                ...
                {offeneAnfragen.length > 0 ? (
                  <div style={{ background: "#FFF8ED", border: "1px solid #FED7AA", borderRadius: 14, padding: 16, marginBottom: 16 }}>
                    <div style={{ fontWeight: 700, color: "#92400E", marginBottom: 10, fontSize: "0.88rem" }}>
                      {offeneAnfragen.length} neue Anfragen
                    </div>
                    {offeneAnfragen.slice(0, 2).map(a => (
                      <div key={a.id} style={{ background: "white", borderRadius: 10, padding: "10px 14px", marginBottom: 8, fontSize: "0.85rem", fontWeight: 600, color: NAVY }}>{a.kita_name}</div>
                    ))}
                  ...
                  <div style={{ background: "#F8FAFF", borderRadius: 14, padding: 20, marginBottom: 16, textAlign: "center" }}>
                    ...
                    <div style={{ color: "#9BA8C0", fontSize: "0.82rem", lineHeight: 1.6 }}>
                      {fachkraft?.aktiv_suchend ? "Dein Profil ist sichtbar." : "Schalte die Jobsuche ein."}
                    </div>
                  </div>
                )}
              ...
              <div>
                {anfragen.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "40px 20px" }}>
                    ...
                    <div style={{ color: "#9BA8C0", fontSize: "0.85rem" }}>Sobald eine Kita Interesse hat, erscheint die Anfrage hier.</div>
                  </div>
                ) : (
                  <>
                    {offeneAnfragen.length > 0 && (
                      <div style={{ marginBottom: 24 }}>
                        <div style={{ fontSize: "0.75rem", fontWeight: 800, color: BLUE, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>Neue Anfragen - {offeneAnfragen.length} offen</div>
                        {offeneAnfragen.map(anfrage => (
                          <div key={anfrage.id} className="anfrage-card" style={{ borderLeft: `4px solid ${BLUE}` }}>
                            ...
                            <div style={{ background: "#FFF8ED", borderRadius: 10, padding: "9px 13px", marginBottom: 14, fontSize: "0.78rem", color: "#92400E" }}>
              ...
              <div>
                {Object.keys(konversationen).length === 0 ? (
                  <div style={{ textAlign: "center", padding: "40px 20px" }}>
                    ...
                    <div style={{ color: "#9BA8C0", fontSize: "0.85rem", lineHeight: 1.7 }}>Wenn eine Kita dir schreibt, erscheint es hier.</div>
                  </div>
                  ...
                  return (
                    <div key={partnerId} style={{ background: "white", borderRadius: 16, padding: 20, marginBottom: 16, border: "1.5px solid #E8EDF4", boxShadow: "0 2px 8px rgba(26,63,111,0.06)" }}>
                      ...
                      {msgSent[partnerId] ? (
                        ...
                        <div>
                          <textarea placeholder={`Antwort an ${agName}...`} rows={2} value={antwort[partnerId] || ""} onChange={e => setAntwort({ ...antwort, [partnerId]: e.target.value })} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleAntwort(partnerId); } }} style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #E2E8F0", borderRadius: 10, fontSize: "0.88rem", resize: "none", fontFamily: "'DM Sans', sans-serif", boxSizing: "border-box", outline: "none" }} />
                          <button onClick={() => handleAntwort(partnerId)} disabled={sendingMsg === partnerId || !antwort[partnerId]?.trim()} style={{ marginTop: 6, width: "100%", padding: "10px", background: sendingMsg === partnerId ? "#ccc" : `linear-gradient(135deg, ${NAVY}, ${BLUE})`, color: "white", border: "none", borderRadius: 9, fontWeight: 700, cursor: "pointer", fontSize: "0.85rem", fontFamily: "'DM Sans', sans-serif" }}>
            ...
            {activeTab === "profil" && form && (
              <div>
                ...
                {fieldGroup("Bundesland", "bundesland", "text", ["Baden-Wuerttemberg","Bayern","Berlin","Brandenburg","Bremen","Hamburg","Hessen","Mecklenburg-Vorpommern","Niedersachsen","Nordrhein-Westfalen","Rheinland-Pfalz","Saarland","Sachsen","Sachsen-Anhalt","Schleswig-Holstein","Thueringen"])}
                {fieldGroup("Qualifikation", "qualifikation", "text", ["Staatlich anerkannte/r Erzieher/in","Kindheitspaedagoge/in","Sozialpaedagoge/in","Heilpaedagoge/in","Sozialarbeiter/in","Paedagogische Fachkraft (auslaendischer Abschluss)","Quereinstieg","Sonstiges"])}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }} className="two-col">
                  {fieldGroup("Verfuegbar ab", "verfuegbar_ab", "date")}
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <textarea name="beschreibung" value={form?.beschreibung || ""} onChange={e => setForm({ ...form, beschreibung: e.target.value })} rows={4} style={{ ...inputStyle, resize: "vertical" }} placeholder="Beschreibe dich kurz..." />
                </div>
                {saveError && <div style={{ marginBottom: 12, padding: "11px 14px", background: "#FFF5F5", borderRadius: 10, color: "#9B1C1C", fontSize: "0.85rem" }}>{saveError}</div>}
            ...
            {activeTab === "konto" && (
              <div>
                ...
                <div style={{ padding: 16, background: "#FFF5F5", border: "1px solid #FED7D7", borderRadius: 12 }}>
                  <div style={{ fontWeight: 700, color: "#9B1C1C", marginBottom: 4, fontSize: "0.9rem" }}>Account loeschen</div>
                  <div style={{ color: "#7F1D1D", fontSize: "0.82rem", marginBottom: 14, lineHeight: 1.6 }}>Dein Account und alle Daten werden unwiderruflich geloescht.</div>
